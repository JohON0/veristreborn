...
public class FirstPersonRenderer {
    ...
    private void transformFirstPerson(MatrixStack matrixStackIn, HandSide handIn, float swingProgress) {
        ModuleManager functionRegistry = Expensive.getInstance().getModuleManager();
        ViewModel swingAnimation = functionRegistry.getViewModel();
    ...
    private void transformSideFirstPerson(MatrixStack matrixStackIn, HandSide handIn, float equippedProg) {
        ModuleManager functionRegistry = Expensive.getInstance().getModuleManager();
        ViewModel swingAnimation = functionRegistry.getViewModel();
        HitAura killAura = functionRegistry.getHitAura();
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn)) {
            ...
            if (stack.isEmpty()) {
                ...
                if (viewModel.isState()) {
                    if (flag3) {
                        matrixStackIn.translate(viewModel.x.get(),
                                viewModel.y.get(), viewModel.z.get());
                    } else {
                        matrixStackIn.translate(viewModel.x.get(),
                                viewModel.y.get(), viewModel.z.get());
                    }
